            ...
            >
              ...
              <div className={`h-32 bg-gradient-to-br ${style.gradient} relative overflow-hidden`}>
                ...
                <div className="absolute bottom-4 left-5">
                  <span className="text-5xl font-bold text-white">{level.id.toUpperCase()}</span>
                </div>
              </div>
              ...
              <div className="p-5 space-y-3">
                ...
                <div className="flex items-center gap-3 pt-2">
                  <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full ${style.light} ${style.text} text-xs font-semibold`}>
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        <button
          onClick={onBack}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white shadow-md hover:shadow-lg text-sm font-semibold text-gray-600 hover:text-emerald-600 transition-all hover:-translate-y-0.5 border border-gray-100"
        >
      ...
      {/* 搜索 */}
      <div className="flex flex-col md:flex-row gap-4 md:items-center md:justify-between bg-white/80 backdrop-blur-sm rounded-2xl p-4 shadow-sm border border-gray-100"
      >
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder="搜索绘本标题或内容..."
            className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 pl-11 text-sm text-gray-700 shadow-inner focus:border-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-100 transition-all"
          />
        </div>
        <p className="text-sm text-gray-500">
          共 <span className={`font-semibold ${style.text}`}>{books.length}</span> 本绘本
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {books.map((book, index) => (
          <motion.button
            ...
            className="group text-left rounded-2xl bg-white p-5 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-100 hover:border-emerald-200"
          >
            <div className="flex items-start gap-4">
              {/* 序号 */}
              <div className={`w-14 h-14 rounded-2xl ${style.light} flex items-center justify-center text-xl font-bold ${style.text} shadow-inner group-hover:scale-110 transition-transform`}>
                {book.episode}
              </div>
...
function BookDetail({ book, onBack }: { book: OxfordBook; onBack: () => void }) {
  ...
  return (
    ...
    >
      {/* 导航栏 */}
      <div className="flex items-center gap-3 bg-white/80 backdrop-blur-sm rounded-2xl p-4 shadow-sm border border-gray-100"
      >
        <button
          onClick={onBack}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-50 hover:bg-emerald-50 text-sm font-semibold text-gray-600 hover:text-emerald-600 transition-all"
        >
          <ArrowLeft className="w-4 h-4" />
          返回绘本列表
        </button>
        <span className="text-gray-300">/</span>
        <span className="text-sm text-gray-500">{book.episode}</span>
